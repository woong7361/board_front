...
import {FREE_BOARD_EDIT_ROUTER_NAME, FREE_BOARD_LIST_ROUTER_NAME} from "@/constant/routeNames.js";
    ...
    <v-col cols="9" class="mx-auto pa-12 pb-8 mb-10">
      ...
      <v-card min-height="800"  elevation="2" variant="flat">
        ...
        <v-card max-width="1100px" min-height="100px" class="mx-auto pa-3 pb-3 mt-10 mb-10" variant="tonal">
          ...
          <comment-form v-if="memberStore.isLogin" :isDisable="false" v-model="commentFormData.content" @submitComment="submitComment" label="댓글 작성"/>
          <comment-form v-else :isDisable="true" label="로그인이 필요합니다."/>
      ...
      <v-card class="mt-5 pa-3" variant="flat">
        <div class="d-flex flex-row-reverse">
          ...
          <route-button v-show="memberStore.memberId === freeBoard.memberId" value="수정" class="ml-1" variant="outlined" @clickEvent="goEdit" size="large"/>
          <button-dialog v-show="memberStore.memberId === freeBoard.memberId" dialog-title="알림" dialog-text="게시글 삭제 실패" v-model:dialog="dialog.board">
